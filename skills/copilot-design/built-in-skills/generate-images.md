...
3. **Auto-select** — inspect your available-skills / tool inventory, in order:
   ...
   - Otherwise, use any installed general-purpose image-generation skill.
4. **None available** — tell the user and ask how to proceed. Do not silently fall back.

Concrete tool names (`imagegen`, `GenerateImage`, `image_generate`) are examples — substitute the local equivalent under the same rule.
...
- **Other installed image-generation skill** — read its `SKILL.md` and follow its documented invocation and output rules.
